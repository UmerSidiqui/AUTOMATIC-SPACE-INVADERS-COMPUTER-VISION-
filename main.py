...
def main():
    ...
    with mphands.Hands(min_detection_confidence=0.8,min_tracking_confidence=0.5) as hands:
        while run:
            ...
            if res==2 and player.x - player_vel > 0:
                player.x -= player_vel
            elif res==3 and player.x + player_vel + player.get_width() < WIDTH:
                player.x += player_vel


            elif  res==4 and player.y - player_vel > 0: # up
                player.y -= player_vel
            elif  res==0 and player.y + player_vel + player.get_height() + 15 < HEIGHT: # down
                player.y += player_vel
